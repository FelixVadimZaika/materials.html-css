	...
	fetch(url)
		...
		.then(people => {
			...
			for (let i = 0; i < people.length; i++) {
				...
				td2.innerHTML = people[i].model;
				tr.appendChild(td2);

				let td3 = document.createElement("th");
				td3.innerHTML = people[i].manufacturer;
				tr.appendChild(td3);

				let td4 = document.createElement("th");
				td4.innerHTML = people[i].cost_in_credits;
				tr.appendChild(td4);

				let td5 = document.createElement("th");
				td5.innerHTML = people[i].length;
				tr.appendChild(td5);

				let td6 = document.createElement("th");
				td6.innerHTML = people[i].max_atmosphering_speed;
				tr.appendChild(td6);

				let td7 = document.createElement("th");
				td7.innerHTML = people[i].passengers;
				tr.appendChild(td7);

				let td8 = document.createElement("th");
				td8.innerHTML = people[i].cargo_capacity;
				tr.appendChild(td8);

				let td9 = document.createElement("th");
				td9.innerHTML = people[i].consumables;
				tr.appendChild(td9);

				let td10 = document.createElement("th");
				td10.innerHTML = people[i].hyperdrive_rating;
				tr.appendChild(td10);

				let td11 = document.createElement("th");
				td11.innerHTML = people[i].MGLT;
				tr.appendChild(td11);

				let td12 = document.createElement("th");
				td12.innerHTML = people[i].starship_class;
				tr.appendChild(td12);
			}
		})
}

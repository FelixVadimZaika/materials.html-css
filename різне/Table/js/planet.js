	...
	fetch(url)
		...
		.then(people => {
			...
			for (let i = 0; i < people.length; i++) {
				...
				td2.innerHTML = people[i].rotation_period;
				tr.appendChild(td2);

				let td3 = document.createElement("th");
				td3.innerHTML = people[i].orbital_period;
				tr.appendChild(td3);

				let td4 = document.createElement("th");
				td4.innerHTML = people[i].diameter;
				tr.appendChild(td4);

				let td5 = document.createElement("th");
				td5.innerHTML = people[i].climate;
				tr.appendChild(td5);

				let td6 = document.createElement("th");
				td6.innerHTML = people[i].gravity;
				tr.appendChild(td6);

				let td7 = document.createElement("th");
				td7.innerHTML = people[i].terrain;
				tr.appendChild(td7);

				let td8 = document.createElement("th");
				td8.innerHTML = people[i].surface_water;
				tr.appendChild(td8);

				let td9 = document.createElement("th");
				td9.innerHTML = people[i].population;
				tr.appendChild(td9);
			}
		})
}
